...
export class TypeValidator {
  ...
  public validateTypeCompatibility(
    ...
    errors.push(`Incompatible types: Cannot connect ${sourcePort.dataType} to ${targetPort.dataType}`);
  ...
  private areTypesCompatible(sourceType: DataType, targetType: DataType): boolean {
    ...
    return compatiblePairs.some(([source, target]) => 
   ...
   */
  public getTypeConverter(sourceType: DataType, targetType: DataType): ((value: any) => any) | null {
    ...
    const converters: Record<string, (value: any) => any> = {
      ...
      [`${DataType.STRING}_${DataType.BOOLEAN}`]: (value) => Boolean(value && value !== 'false' && value !== '0'),
      ...
      [`${DataType.JSON}_${DataType.OBJECT}`]: (value) => typeof value === 'string' ? JSON.parse(value) : value,
      [`${DataType.JSON}_${DataType.ARRAY}`]: (value) => typeof value === 'string' ? JSON.parse(value) : value,
      [`${DataType.JSON}_${DataType.STRING}`]: (value) => typeof value === 'string' ? value : JSON.stringify(value),
    };

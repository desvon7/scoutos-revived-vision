...
import { 
  ...
  ExecutionState, NodeExecutionState, DataType
} from '../types/nodes';
...
class WorkflowEngine {
  ...
  public async executeWorkflow(
    ...
    if (options.timeout) {
      this.executionTimeouts.set(executionId, setTimeout(() => {
  ...
  private async processDownstreamNodes(
    ...
    for (const targetNodeId of targetNodeIdsArray) {
      ...
      const incomingConnections = workflow.edges.filter(
        conn => conn.target === targetNodeId
      );
